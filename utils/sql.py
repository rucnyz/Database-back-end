...
class Database:
    ...
    def __init__(self, dsn_name: str):
        ...
        self.connect_pool = QueuePool(
            self.__get_connection, pool_size=100, max_overflow=20)
...
def run_sql(T_sql: str, param: dict = None, user="sa"):  # 把T_sql放到sql server中运行
    return user_dict[user].query(T_sql, param)


def run_sqls(sqlList: List[dict[str, dict]], user="sa"):  # 把T_sql放到sql server中运行
    result = {}
    for T_sql, param in sqlList:
